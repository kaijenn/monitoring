    ...
    public function tampil1($yoga)
{
    ...
    public function tampilWhere($yoga, $whereCondition)
    {
        return $this->db->table($yoga)
            ...
            ->where('deleted_at', null) // Menambahkan kondisi deleted_at IS NULL
            ->get()
            ->getResult();
    }
    ...
    public function carikelas($tgl_awal, $tgl_selesai)
{
    $query = "SELECT kelas.nama_kelas,
                     ...
                     kelas.id_penanggung,
                     kelas.tanggal,
                     user_penanggung.username AS penanggung_username
              FROM kelas
              JOIN user AS user_penanggung ON kelas.id_penanggung = user_penanggung.id_user
                WHERE kelas.tanggal BETWEEN ? AND ?";
...
{
    // Pastikan tabel dan kolom sesuai
    return $this->db->table($tabel)
        ->where('id', $isi['id'])  // Pastikan kondisi WHERE sesuai
        ->update(['status' => $isi['status']]);
}
    ...
    public function joinKelasWithUser($id_user)
    {
        return $this->db->table('kelas')
            ->join('user', 'kelas.id_user = user.id_user', 'inner')
            ->where('kelas.id_user', $id_user)
            ->get()
            ->getResult();
    }
    public function joinKelasWithUser1($where)
{
    return $this->db->table('kelas')
        ->select('kelas.*, user.*, kelas.status as kelasstatus')
        ->join('user', 'kelas.id_penanggung = user.id_user', 'inner')
        ->where($where)
        ->get()
        ->getResult();
}
    ...
    public function getBackupData1($table, $id)
    {
        ...
        return $this->db->table($table)->where('id_kelas', $id)->get()->getRowArray();
    }
...
public function deleteLogById($id_log)
{
    // Ensure the id_log is provided and is valid
    if (!empty($id_log)) {
        // Delete the log entry for the given log ID
        $builder = $this->db->table('user_activity');
        $builder->where('id_log', $id_log); // Use 'id_log' instead of 'id_user'
...
public function countKelasKosong()
    {
        $builder = $this->db->table('kelas'); // Ganti 'kelas' dengan nama tabel Anda
        return $builder->where('status', 'Kosong')->countAllResults();
    }
    public function countKelasPenuh()
    {
        $builder = $this->db->table('kelas'); // Ganti 'kelas' dengan nama tabel Anda
        return $builder->where('status', 'Sedang di Pakai')->countAllResults();
    }
    public function countKelas()
    {
        $builder = $this->db->table('kelas'); // Nama tabel Anda
        return $builder->countAllResults(); // Menghitung semua entri
    }
